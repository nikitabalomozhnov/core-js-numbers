...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const sumProducts = x1 * x2 + y1 * y2;
  const magnitudeFirst = x1 * x1 + y1 * y1;
  const magnitudeSecond = x2 * x2 + y2 * y2;
  return Math.acos(sumProducts / (magnitudeFirst * magnitudeSecond));
}
...
function roundToPowerOfTen(num, pow) {
  ...
  if (pow === 1) {
    if (temp >= 5) {
      return Math.ceil(num - (num % 10 ** pow) + 10 ** pow);
    }
  ...
  return temp >= 500
    ? Math.ceil(num - (num % 10 ** pow) + 10 ** pow)
    : num - (num % 10 ** pow);
}
